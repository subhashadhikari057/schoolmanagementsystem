...
export default function AcademicStatsPanel({
  data,
  title = 'Academic Stats',
}: AcademicStatsPanelProps) {
  const academicStats = [
    {
    ...
    <div className='space-y-3'>
      ...
      <div className='grid grid-cols-2 gap-2 sm:gap-3'>
        {academicStats.map((stat, index) => (
          <div
            key={index}
            className='bg-white rounded-xl border border-gray-200 p-2 sm:p-3 lg:p-4 shadow-sm hover:shadow-md transition-shadow min-w-0'
          >
            <div className='space-y-1 sm:space-y-2 min-w-0 flex-1'>
              <Label className='text-xs sm:text-sm text-gray-600 truncate'>
                {stat.label}
              </Label>
              <Metric
                value={stat.value}
