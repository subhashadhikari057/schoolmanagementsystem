...
import BarLegend from "@/components/molecules/interactive/BarLegend";
 const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-gray-200 text-sm text-gray-800 px-3 py-1 rounded shadow" style={{ width: "fit-content" }}>
...
export default function AttendanceOverview() {
    return (
            <ChartCard className='p-12'>
                ...
                <BarChart data={data} margin={{ top: 20, right: 0, left: 0, bottom: 5 }} barCategoryGap="20%">
                <XAxis 
                  dataKey="day" 
                  axisLine={false} 
                  tickLine={false}
                ...
                <Bar radius={[3,3,0,0]} dataKey="students" stackId="a" fill="#3b82f6" maxBarSize={35} />
                <Bar radius={[3,3,0,0]} dataKey="teachers" stackId="a" fill="#d1d5db" maxBarSize={35} />
                </BarChart>
                </ResponsiveContainer>
               <div className='mt-5'> <BarLegend items={[{ label: "Students", color: "#3b82f6" }, { label: "Teachers", color: "#d1d5db" }]} /></div>
            </ChartCard>
    );
}