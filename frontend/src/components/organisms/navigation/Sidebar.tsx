...
export default function Sidebar({ isOpen = false, onToggle }: SidebarProps) {
  ...
  // Map backend roles to sidebar keys
  const mapRoleToSidebarKey = (userRole: string): string => {
    ...
    return roleMap[userRole] || 'Superadmin';
  };

  const role = user ? mapRoleToSidebarKey(user.role) : 'Superadmin';
