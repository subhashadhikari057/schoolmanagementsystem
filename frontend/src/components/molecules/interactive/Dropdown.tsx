...
export default function Dropdown({
          ...
          <Menu>
            {({ open }: { open: boolean }) => (
              <div className='w-full'>
                {/* Profile Button */}
                <Menu.Button
                  className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 md:py-3 w-full min-w-[180px] sm:min-w-[200px] bg-white rounded-lg transition-all ${
                ...
                >
                  ...
                  {open ? (
                    <ChevronUp
                      size={14}
                      className='sm:w-4 sm:h-4 text-secondary ml-auto flex-shrink-0'
                    />
                  ) : (
                    <ChevronDown
                      size={14}
                ...
                >
                  <Menu.Items className='absolute top-full left-0 mt-1 w-full min-w-[180px] sm:min-w-[200px] bg-white border border-gray-200 rounded-b-lg shadow-lg z-50'>
                        ...
                        <Menu.Item key={option.value}>
                          {({ active }: { active: boolean }) => (
                            <button
                              ...
                              className={`flex items-center gap-2 sm:gap-3 w-full px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm transition-colors ${
                                active
  ...
  return (
    <div
      ref={dropdownRef}
      className={`relative inline-block text-left min-w-[140px] sm:min-w-[160px] w-full ${className || ''}`}
    >
      {/* Filter Button */}
      <button
        onClick={toggleFilterDropdown}
        className='flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-sm hover:bg-blue-50 focus:ring-2 focus:ring-blue-200 transition-all text-xs sm:text-sm w-full min-w-[140px] sm:min-w-[160px] max-w-xs font-medium outline-none'
        type='button'
      >
        ...
        {isFilterOpen ? (
          <ChevronUp
            size={16}
            className='sm:w-[18px] sm:h-[18px] text-gray-500 transition-transform flex-shrink-0'
          />
        ) : (
          <ChevronDown
            size={16}
      ...
      {isFilterOpen && (
        <div className='absolute top-full left-0 mt-1 sm:mt-2 w-full min-w-[140px] sm:min-w-[160px] max-w-xs bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-xl z-50 animate-fade-in'>
          <div className='py-1'>
            {title && (
              <div className='px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100'>
                {title}
              </div>
            )}
            {currentOptions.map(option => (
              <button
                ...
                className={`flex items-center w-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm rounded-lg transition-all duration-150 ${
              ...
              >
                ...
                {selectedValue === option.value && (
                  <div className='ml-2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-600 rounded-full flex-shrink-0'></div>
                )}
              </button>
            ))}
          </div>
