    ...
    <div className={className}>
      ...
      <div className='lg:grid lg:grid-cols-4 lg:gap-6 flex lg:flex-none gap-3 overflow-x-auto lg:overflow-x-visible scrollbar-hide pb-3 lg:pb-0'>
        {systemMetrics.map(metric => (
          <div
            key={metric.id}
            className='bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-5 min-w-[160px] lg:min-w-0 flex-shrink-0 lg:flex-shrink hover:shadow-md transition-shadow'
          >
            <div className='flex flex-col items-center text-center space-y-3'>
              {/* Icon with background */}
              <div
                className={`w-12 h-12 lg:w-14 lg:h-14 rounded-full bg-gray-50 flex items-center justify-center ${metric.color.replace('text-', 'bg-').replace('-500', '-50')}`}
              >
                <Icon>
