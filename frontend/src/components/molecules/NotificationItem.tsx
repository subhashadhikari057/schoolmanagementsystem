...
export default function NotificationItem({notification,onMarkAsRead}:{notification: Notification,onMarkAsRead: (id: string) => void}) {
    const iconStyles = {
        info : "",
        warning : "",
        success : "",
  ...
  return (
    ...
    >
      <Icon className={iconStyles[notification.type]}>
        {notification.icon}
      </Icon>

      <div className="flex-1 min-w-0">
        <h3 className={`font-medium mb-1 ${notification.read ? 'text-gray-700' : 'text-gray-900'}`}>
          {notification.title}
        </h3>
