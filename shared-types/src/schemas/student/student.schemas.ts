...
export const CreateStudentRequestSchema = z.object({
  ...
  roll_number: z.string()
    ...
    .regex(/^[A-Z0-9-]+$/, 'Roll number can only contain uppercase letters, numbers, and hyphens'),
  ...
  medical_info: z.object({
    ...
    medical_conditions: z.string().max(500, 'Medical conditions description too long').optional(),
  }).optional(),
 ...
 */
export const PaginatedStudentsResponseSchema = createPaginatedResponseSchema(StudentResponseSchema);
...
export const StudentProfileResponseSchema = StudentResponseSchema.extend({
  ...
  recent_attendance: z.array(z.object({
    date: z.date(),
    status: z.string(),
    remarks: z.string().optional(),
  })).optional(),
  recent_assignments: z.array(z.object({
    id: CommonValidation.uuid,
    title: z.string(),
    due_date: z.date(),
    status: z.string(),
  ...
  parent_contacts: z.array(z.object({
    id: CommonValidation.uuid,
    full_name: z.string(),
    email: z.string(),
...
export const BulkCreateStudentsSchema = z.object({
  students: z.array(CreateStudentRequestSchema)
...
export const BulkUpdateStudentsSchema = z.object({
  student_ids: z.array(CommonValidation.uuid)
...
export const PromoteStudentSchema = z.object({
  ...
  to_class_id: CommonValidation.uuid,
  to_section_id: CommonValidation.uuid,
  academic_year: z.string().regex(/^\d{4}-\d{4}$/, 'Academic year must be in YYYY-YYYY format'),
