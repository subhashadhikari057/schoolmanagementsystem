...
export const ErrorCodeSchema = z
  ...
  .regex(
    ...
    "Error code must be UPPER_CASE with underscores, no trailing underscore",
  )
 ...
 */
export const ErrorTimestampSchema = z
  .string()
  .refine((val) => {
...
export const ValidationErrorDetailSchema = z.object({
  ...
  message: z
    ...
    .describe("Human-readable validation error message"),
...
export const DatabaseErrorDetailSchema = z.object({
  ...
  constraint: z
    .string()
    .optional()
    .describe("Database constraint that was violated"),
  operation: z
    .enum(["CREATE", "READ", "UPDATE", "DELETE"])
    .optional()
    .describe("Database operation that failed"),
  query: z
    .string()
    .optional()
    .describe("Sanitized query that caused the error (no sensitive data)"),
});
...
export const BusinessLogicErrorDetailSchema = z.object({
  ...
  suggestion: z
    .string()
    .optional()
    .describe("Suggested action to resolve the error"),
});
...
export const AuthErrorDetailSchema = z.object({
  ...
  remainingAttempts: z
    ...
    .min(0)
    .optional()
    .describe("Remaining login attempts before lockout"),
  lockoutUntil: z
    .string()
    .datetime()
    .optional()
    .describe("Account lockout expiration time"),
});
...
export const RateLimitErrorDetailSchema = z.object({
  ...
  remaining: z
    ...
    .int()
    .min(0)
    .describe("Remaining requests in current window"),
  resetTime: z
    .string()
    .datetime()
    .describe("When the rate limit window resets"),
  retryAfter: z
    .number()
    .int()
    .min(0)
    .describe("Seconds to wait before retrying"),
});
...
export const FileErrorDetailSchema = z.object({
  filename: z
    .string()
    .optional()
    .describe("Name of the file involved in the error"),
  ...
  maxSize: z
    ...
    .min(0)
    .optional()
    .describe("Maximum allowed file size"),
  allowedTypes: z
    .array(z.string())
    .optional()
...
export const ExternalServiceErrorDetailSchema = z.object({
  ...
  endpoint: z
    .string()
    .optional()
    .describe("External service endpoint that failed"),
  statusCode: HttpStatusCodeSchema.optional().describe(
    "HTTP status code from external service",
  ),
  timeout: z
    .boolean()
    .default(false)
    .describe("Whether the error was due to timeout"),
});
...
export const BaseErrorResponseSchema = z.object({
  ...
  error: z
    ...
    .describe("HTTP status text or error name"),
  message: z
    .string()
  ...
  timestamp: ErrorTimestampSchema.optional().describe(
...
export const DetailedErrorResponseSchema = BaseErrorResponseSchema.extend({
  details: z
    .object({
      ...
      database: DatabaseErrorDetailSchema.optional().describe(
        "Database error details",
      ),
      business: BusinessLogicErrorDetailSchema.optional().describe(
        "Business logic error details",
      ),
      auth: AuthErrorDetailSchema.optional().describe(
        "Authentication error details",
      ),
      rateLimit: RateLimitErrorDetailSchema.optional().describe(
        "Rate limiting error details",
      ),
      file: FileErrorDetailSchema.optional().describe(
  ...
  stack: z
    .string()
    .optional()
    .describe("Error stack trace (only in development)"),
  ...
  context: z
    .object({
      userId: z
        ...
        .uuid()
        .optional()
        .describe("ID of the user who triggered the error"),
      userRole: z
        .string()
        .optional()
        .describe("Role of the user who triggered the error"),
      endpoint: z
        .string()
        .optional()
        .describe("API endpoint that generated the error"),
      method: z
    ...
    .optional()
    .describe("Request context information"),
});
...
export type BusinessLogicErrorDetail = z.infer<
  typeof BusinessLogicErrorDetailSchema
>;
...
export type ExternalServiceErrorDetail = z.infer<
  typeof ExternalServiceErrorDetailSchema
>;
