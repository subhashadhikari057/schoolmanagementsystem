...
export const CreateGradingScaleDto = z.object({
  ...
  gradeDefinitions: z.array(GradeDefinitionSchema).min(1, 'At least one grade definition is required'),
});
...
export const BulkGridGradingDto = z.object({
  ...
  grades: z.array(GridGradeEntryDto).min(1, 'At least one grade entry is required'),
});
...
export type GetStudentGradeHistoryDtoType = z.infer<typeof GetStudentGradeHistoryDto>;
