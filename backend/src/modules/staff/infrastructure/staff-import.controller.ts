...
@ApiTags('Staff Import/Export')
@Controller('api/v1/staff-import')
@UseGuards(JwtAuthGuard, RolesGuard)
export class StaffImportController {
  private readonly logger = new Logger(StaffImportController.name);
  ...
  @ApiOperation({ summary: 'Import staff from XLSX file' })
    ...
    schema: {
      ...
      properties: {
        file: {
          type: 'string',
          format: 'binary',
          description: 'XLSX file with staff data',
        },
        skipDuplicates: {
          type: 'boolean',
          description: 'Skip duplicate staff instead of failing',
          default: false,
        },
        updateExisting: {
          type: 'boolean',
          description: 'Update existing staff instead of failing',
          default: false,
        },
      },
    },
  ...
  async importStaff(
    ...
    return await this.staffImportService.importStaffFromRecords(
      records,
      user?.id || 'system',
      options,
    );
  ...
  @RoleAccess.AdminLevel()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Export staff to XLSX' })
  @ApiResponse({
    status: 200,
    description: 'XLSX file with staff data',
    schema: {
  ...
  async exportStaff(
    ...
    const { headers, rows } = await this.staffImportService.exportStaffToRows({
      department,
      search,
      designation,
      employmentStatus,
    ...
    const worksheet = workbook.addWorksheet('Staff Export');
  ...
  @RoleAccess.AdminLevel()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Get XLSX template for staff import' })
  @ApiResponse({
    status: 200,
  ...
  async getImportTemplate(@Res() res: Response) {
    const { headers, sampleRow } =
      this.staffImportService.getImportTemplateData();
    const ExcelJS = await import('exceljs');
    const workbook = new ExcelJS.Workbook();
