import {
  ...
  import { AssignSubjectsDto, AssignSubjectsDtoType } from '../dto/assign-subjects.dto';
  ...
  export class TeacherController {
    ...
    async create(
      ...
      const result = await this.teacherService.create(body, user.id, req.ip, req.headers['user-agent']);
      return {
  ...
  @UseGuards(hasRole('TEACHER'))
  async updateSelf(
    @Body(new ZodValidationPipe(UpdateTeacherSelfDto)) body: UpdateTeacherSelfDtoType,
    @CurrentUser() user: any,
    @Req() req: Request,
  ) {
    return this.teacherService.updateSelf(user.id, body, req.ip, req.headers['user-agent']);
  }
  ...
  async update(
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateTeacherByAdminDto)) body: UpdateTeacherByAdminDtoType,
    @CurrentUser() user: any,
    @Req() req: Request,
  ) {
    return this.teacherService.updateByAdmin(id, body, user.id, req.ip, req.headers['user-agent']);
  }
    ...
    async remove(
      ...
      return this.teacherService.softDelete(id, user.id, req.ip, req.headers['user-agent']);
    }
    ...
    async assignSubjects(
      @Param('id') id: string,
      @Body(new ZodValidationPipe(AssignSubjectsDto)) body: AssignSubjectsDtoType,
      @CurrentUser() user: any,
      @Req() req: Request,
    ) {
      return this.teacherService.assignSubjects(id, body.subjectIds, user.id, req.ip, req.headers['user-agent']);
    }
    ...
    async unassignSubject(
      ...
      return this.teacherService.removeSubject(id, subjectId, user.id, req.ip, req.headers['user-agent']);
    }
