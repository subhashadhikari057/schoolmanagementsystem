...
describe("Docker Stack Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      `MailHog: ${TEST_CONFIG.mailhog.smtp.host}:${TEST_CONFIG.mailhog.smtp.port}`,
    );
    console.log("");
  ...
  describe("Remote PostgreSQL Database Integration", () => {
    ...
    test("should have required database extensions", async () => {
      ...
      console.log(
        `✅ Database extensions available: ${(extensions as any[]).map((e) => e.extname).join(", ")}`,
      );
    });
  ...
  describe("Local Redis Cache Integration", () => {
    ...
    test("should be able to set and get values", async () => {
      ...
      console.log(
        `✅ Redis operations: SET/GET verified with key "${testKey}"`,
      );
  ...
  describe("Remote Database Schema Validation", () => {
    test("should verify database schema exists", async () => {
      try {
        ...
        console.log(
          `✅ Database schemas available: ${(schemas as any[]).map((s) => s.schema_name).join(", ")}`,
        );
      } catch (error) {
...
export function testDockerStackConnectivity() {
  ...
  return checkDockerServices().then((results) => {
    console.log(
      `PostgreSQL (Remote): ${results.postgres ? "✅ Connected" : "❌ Failed"}`,
    );
    console.log(
      `Redis (Local): ${results.redis ? "✅ Connected" : "❌ Failed"}`,
    );
    console.log(
      `MailHog (Local): ${results.mailhog ? "✅ Connected" : "❌ Failed"}`,
    );

    const allHealthy = results.postgres && results.redis && results.mailhog;
    console.log(
      `Overall Status: ${allHealthy ? "✅ All services healthy" : "⚠️ Some services unavailable"}`,
    );
